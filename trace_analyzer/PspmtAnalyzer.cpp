...
int PspmtAnalyzer::Process(std::vector<processor_struct::PSPMT> &pspmt_vec,const ULong64_t ts){
   ...
   for(const auto &dyn_high: dynode_high_vec){
      ...
      {
         pspmt_data_.pid_pin0_=-999;
         pspmt_data_.pid_tac1_=-999;
         pspmt_data_.pid_tof1_=-999;
         /* pid information */
         if(pid_data_.size()>0){
            if(pid_data_[0].pin_0_energy>0){
               pspmt_data_.pid_pin0_ = pid_data_[0].pin_0_energy;
               pspmt_data_.pid_tac1_ = pid_data_[0].tac_1;
               pspmt_data_.pid_tof1_ = pid_data_[0].tof1;
            }
         }
      }
...
void PspmtAnalyzer::CalculatePositionL(pspmt_data_struc &data)
{
   ...
   const double yb = data.yb_.pspmt_.qdc;
   //added by Xu
   const double dy = data.dynode_.pspmt_.qdc;//dynode qdc, for correction
