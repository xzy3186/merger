...
int main(int argc, char** argv)
{
	...
	try {
		...
		std::cout << "[AnamergerSparseMain]: Number of TTrees added to the chain: " << chain->GetNtrees() << std::endl;
		...
		if (use_proof) {
			...
			chain->Process("AnamergerSparseSelector", "", n_entries, first_entry);
		}
		else {
			std::cout << "Start Processing (Proof OFF)..." << std::endl;
			AnamergerSparseSelector* selector = new AnamergerSparseSelector(chain);
